...
exports.login = async ( req, res ) => {
  ...
  try {
    ...
    if ( !existingUser || !( await bcrypt.compare( password, existingUser.password ) ) ) {
      return res.status( 401 ).json( { message: !existingUser ? 'Account does not exist with this email' : 'Incorrect password' } );
    }
...
exports.restoreUser = async ( req, res ) => {
  try {
    const existingUser = req.user; // The user will be attached by the auth middleware
    res.status( 200 ).json( { user: { name: existingUser.name, email: existingUser.email } } );
  } catch ( error ) {
